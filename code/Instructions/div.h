...
/**
 * @brief Class that represents the Div instruction. The instruction divides two
 * operands and stores the result in a register.
 */
class Div : public Instruction {
 public:
  Div() {}
  Div(const string &line) {
    // Parse the line
  ...
  void execute(vector<double> &memory, vector<string> &stringMemory,
    ...
    if (operands_.size() != 3) {
      throw invalid_argument("div instruction must have 3 operands");
      return;
    }
    double operand_1{0};
    double operand_2{0};
    ...
    if (operands_[0].getAddressingMode() == 2) {  // Direct addressing
      ...
      throw invalid_argument(
          "div instruction cannot have immediate addressing mode");
    }
  }

  string getName() const override { return "div"; }
