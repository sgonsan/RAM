...
/**
 * @brief Class that represents the Sub instruction. The instruction subtracts
 * the value of a register from another register and stores the result in a
 * register.
 */
class Sub : public Instruction {
 public:
  Sub() {}
  Sub(const string &line) {
    // Parse the line
  ...
  void execute(vector<double> &memory, vector<string> &stringMemory,
    ...
    if (operands_.size() != 3) {
      throw invalid_argument("sub instruction must have 3 operands");
      return;
    }
    double operand_1{0};
    double operand_2{0};
    ...
    if (operands_[1].getAddressingMode() == 2) {  // Direct addressing
      if (operands_[1].getValue() >= registers.size() ||
        ...
        throw invalid_argument(
            "register " + to_string(operands_[1].getValue()) + " not found");
      }
    ...
    if (operands_[2].getAddressingMode() == 2) {  // Direct addressing
      if (operands_[2].getValue() >= registers.size() ||
        ...
        throw invalid_argument(
            "register " + to_string(operands_[2].getValue()) + " not found");
      }
    ...
    if (operands_[0].getAddressingMode() == 2) {  // Direct addressing
      ...
      throw invalid_argument(
          "sub instruction cannot have immediate addressing mode");
    }
  }

  string getName() const override { return "sub"; }
